...
class New_episode_parameters(JsonObject):
...
class Agent_tracking:
    ...
    def __new_step__(self, step: Step):
        try:
    ...
    def new_episode(self, subject: str, experiment: str, episode: int, occlusions: str, destination_folder: str, register: bool = True, call_back=None):
        if register:
            self.register_consumer(call_back)
        if self.registered:
            body = New_episode_parameters(subject, experiment, episode, occlusions, destination_folder)
            if self.__run__("new_episode", parameters=str(body), new_connection=False, leave_open=True):
                self.episode_in_progress = True
        else:
            return False
        return True

    def end_episode(self, unregister: bool = True):
    ...
    def reset_cameras(self) -> bool:
        return self.__run__("reset_cameras")

    def new_experiment(self, name: str) -> bool:
        return self.__run__("new_experiment", name)

    def update_backgrounds(self) -> bool:
        return self.__run__("update_background")

    def show_occlusions(self, occlusions_configuration: str) -> bool:
        return self.__run__("show_occlusions", occlusions_configuration)

    def hide_occlusions(self) -> bool:
...
class Filtered_client:
    ...
    def filtered_locations(self, previous_location: Location, current_reading: Location) -> Location:
